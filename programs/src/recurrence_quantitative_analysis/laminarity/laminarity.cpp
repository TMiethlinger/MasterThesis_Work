...
string outputfolder_parent = "/home/k3501/k354524/master_thesis_work/results/recurrence_quantitative_analysis/laminarity/";
...
int main(int argc, char * argv[])
{
    ...
    ("outputfilename", value<string>()->default_value("laminarity.txt/"), "Output filename");
    ...
    // For each rank and its jobs, save the result of the computation into laminarity_results
    VVD laminarity_results(njobs_my_rank);
    ...
    for(size_t i = 0; i != njobs_my_rank; i++)
    {
        ...
        lam_vector = rqa_core::laminarity(recurrence_matrix);
        laminarity_results[i] = lam_vector;
    }

    // Write the intermediate laminarity 
    // io_util::write_matrix_result(outputfolder_parent + outputfolder_relative + "result_" + std::to_string(world_rank) + ".txt", laminarity_results);

    // Rank 0 needs to compose the laminarity_results
    if(world_rank == 0)
    {
        // Save results from each rank into the following vector
        VVD total_laminarity_results(njobs, VD(n, 0.0));
        ...
        for(size_t i = 0; i < njobs_my_rank; i++)
        {
            total_laminarity_results[i] = laminarity_results[i];
        }
        ...
        for(int w = 1, j = njobs_my_rank; w < world_size; w++)
        {
            ...
            {
                MPI_Recv(total_laminarity_results[j].data(), n, MPI_DOUBLE, w, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
        ...
        for(int i = 0; i < neps; i++)
        {
            ...
            for(int j = 0; j < n; j++)
            {
                output_result_matrix[i][j + 1] = total_laminarity_results[i][j];
            }
        }

        // Write the computed laminarity vector rates into this filepathname
    ...
    {
        // Therefore, for each epsilon, we send a laminarity vector
        for(size_t i = 0; i != njobs_my_rank; i++)
        { 
            // laminarity_results[i].size() == n
            MPI_Send(laminarity_results[i].data(), n, MPI_DOUBLE, 0, i, MPI_COMM_WORLD);
        }
